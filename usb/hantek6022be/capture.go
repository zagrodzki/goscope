...
var sampleBuf = make([]byte, 8*3*1024)
...
func (h *Scope) getSamples(ep reader, p captureParams, ch chan<- []scope.ChannelData) error {
	...
	for ch := 0; ch < h.numChan; ch++ {
		out := make([]scope.Voltage, num/h.numChan)
		translate := p.translateSample[ch]
		for dst, src := 0, ch; src < num; dst, src = dst+1, src+h.numChan {
			out[dst] = translate[sampleBuf[src]]
		}
		samples[ch] = out
	}
...
func (h *Scope) Start() {
	...
	ep, err := h.dev.InEndpoint(usbInterface, usbAlt, usbEP)
	if err != nil {
	...
	params := captureParams{
		translateSample: make([][]scope.Voltage, 2),
	}
	...
	for idx := range params.translateSample {
		params.translateSample[idx] = make([]scope.Voltage, 256)
		for i := 0; i < 256; i++ {
	...
	if err != nil {
		h.rec.Error(errors.Wrap(err, "NewStream"))
		close(ret)
		return
	}
	go func() {
